...
>
>    **Stepper (above the detail panel):** three steps in a horizontal row, equal-width grid, **max-width 720px, left-aligned to the section's content column** (`margin: 0` — not centered). Each step is left-aligned within its cell (numeral and label both flush-left). Step content: a Cormorant Garamond italic numeral (01 / 02 / 03) at **42px weight 400**, color `--color-rule` for inactive / `--color-gold` for active; below the numeral, a small DM Sans label at **11px weight 400 letter-spacing 0.30em uppercase**, color `--color-warm-gray` for inactive / `--color-navy` for active. Step labels: `24 / 7 ACCESS`, `ACUTE RESPONSE`, `LONG-TERM STEWARDSHIP`. Beneath the stepper row, a hairline rule (`0.5px solid var(--color-rule)`) spans the stepper's full 720px width. **Active indicator:** a gold underline (`1px solid var(--color-gold)`) sits on the active step beneath the rule and slides horizontally between steps on click — `transition: right 500ms cubic-bezier(0.4, 0, 0.2, 1)` (or equivalent transform). Implement as a single sliding pseudo-element or a positioned bar driven by step index — not three separate underlines toggling. Cursor pointer on inactive steps, default on active. Keyboard: Tab to step, Enter/Space to activate.
>
>    **Detail panel (below the stepper):** single panel, **max-width 720px, left-aligned to the section's content column** (`margin: 0`), min-height 320px so the page doesn't reflow as the user steps through. Renders the active pillar's full content: title (Cormorant 32px navy weight 500), subhead (italic Cormorant 22px navy weight 500, with the v3.5 block-margin-bottom rule), one or more body paragraphs (`--text-body` 17px line-height 1.65, **text-align: left**), and the "What this replaces" gold-edge banner (unchanged from v3.5: `--text-replaces` 16px italic, gold left-edge, `--color-cream-dark` background, "What this replaces." label 13px weight 600 letter-spacing 0.18em as a block). **Content swap:** when a step is clicked, fade the detail panel to `opacity: 0` over 300ms, swap the rendered pillar, then fade back to `opacity: 1` over 300ms. Total swap ~600ms.
>
...
| `<PillarStepper />` | Approach | **v3.9 — replaces the prior `<PillarFull />` stacked layout.** Hairline stepper above a single detail panel, **both left-aligned to the section's content column (`margin: 0`, not centered).** Stepper row: three equal-width steps in a 720px-max grid, each step left-aligned within its cell, numeral (italic Cormorant 42px weight 400, `--color-rule` inactive / `--color-gold` active) over a small DM Sans label (11px / 400 / 0.30em / uppercase, `--color-warm-gray` inactive / `--color-navy` active). A 0.5px hairline rule spans the 720px stepper width beneath the steps; a 1px gold active-indicator slides horizontally between steps on click (500ms cubic-bezier ease). Detail panel below: 720px max, left-aligned, min-height 320px, renders the active pillar's full content with v3.5 typography (title 32px Cormorant weight 500; subhead 22px italic Cormorant weight 500, block, margin-bottom 14px; body 17px line-height 1.65 text-align left; "What this replaces" gold-edge banner with the v3.5 13px/600/0.18em label as a block). Content swap: fade out 300ms → swap → fade in 300ms (600ms total). Mobile (<720px): the stepper collapses to a vertical accordion — three full-width rows with numeral + label + chevron, tap to expand height beneath, 400ms ease. Reference visual: `Lewis_Select_Accordion_Concepts.html` Option 2. |
